...
public class NullableResizableArrayImpl<E> implements RandomAccessable<E> {

    protected int size;
    protected E[] entries;
    protected final double ratio;

    public NullableResizableArrayImpl() {
        this(DefaultValues.DEFAULT_CAPACITY);
    }

    public NullableResizableArrayImpl(int cap) {
        this(cap, DefaultValues.CAPACITY_EXPANSION_RATIO);
    }

    public NullableResizableArrayImpl(int cap, double ratio) {
    ...
    public E set(int i, E item) {
        RangeUtil.requireIntRange(i, 0, entries.length);
        E entry = entries[i];
        entries[i] = item;
        return entry;
    }
